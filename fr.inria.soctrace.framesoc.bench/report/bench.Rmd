...
## Trace import with index on event id (EVENT_PARAM table)

As above, but adding the factor of indexing or not the event id on EVENT_PARAM table.
...
# levels(eindex$index) is "false" "true" 
# so all true will be assigned with both and all false with none
# the assignment eindex$index[eindex$index=="true"] <- "both" is not necessary
levels(eindex$index) <- c("none", "both", "time")
